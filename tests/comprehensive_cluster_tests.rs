...
impl ClusterTestSuite {
    ...
    async fn create_node(
        ...
        if !is_bootstrap {
            config.network.bootstrap_peers =
                vec![format!("/ip4/127.0.0.1/tcp/{}", self.config.bootstrap_port)];
        }
    ...
    async fn start_node_network(
        &mut self,
        node: &mut TestNode,
    ) -> Result<(), Box<dyn std::error::Error>> {
        info!(
            "Starting network for node {} on port {}",
            node.id, node.port
        );
    ...
    pub async fn run_comprehensive_tests(&self) -> Result<TestResults, Box<dyn std::error::Error>> {
        ...
        info!(
            "Comprehensive tests completed. Passed: {}, Failed: {}",
    ...
    async fn test_basic_file_operations(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (i, node) in self.nodes.iter().enumerate() {
            ...
            if self
                .simulate_file_store(node, &filename, test_content.as_bytes())
                .await
                .is_ok()
            {
                self.record_test_success("file_store").await;
            } else {
                self.record_test_failure("file_store", "Failed to store file")
                    .await;
            }
            ...
            if self.simulate_file_retrieve(node, &filename).await.is_ok() {
                ...
                self.record_test_failure("file_retrieve", "Failed to retrieve file")
                    .await;
            }
        }
    ...
    async fn test_data_replication(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        self.simulate_file_store(source_node, test_file, test_content)
            .await?;
        ...
        for node in self.nodes.iter().skip(1) {
            if self
                .simulate_file_exists(node, test_file)
                .await
                .unwrap_or(false)
            {
        ...
        if successful_replicas >= self.config.replication_factor - 1 {
            ...
            self.record_test_failure(
                "replication_factor",
                &format!(
                    "Only {} replicas found, expected {}",
                    successful_replicas, self.config.replication_factor
                ),
            )
            .await;
        }
        ...
        for i in 0..nodes_to_fail {
            ...
            if node_index < self.nodes.len() {
                    ...
                    .is_ok()
                {
                    ...
                    self.record_test_failure(
                        "fault_tolerance_operations",
                        "Operations failed during node failure",
                    )
                    .await;
                }
            }
    ...
    async fn test_performance_under_load(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Record performance metrics
        {
            ...
            results
                .performance_metrics
                .insert("ops_per_second".to_string(), ops_per_second);
            results.performance_metrics.insert(
                "load_test_duration_ms".to_string(),
        ...
        if operations_completed == self.config.concurrent_operations {
            ...
            self.record_test_failure(
                "performance_load",
                &format!(
                    "Only {}/{} operations completed",
                    operations_completed, self.config.concurrent_operations
                ),
            )
            .await;
        }
    ...
    async fn test_network_partitioning(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        debug!(
            "Simulating network partition with {} nodes in each partition",
            partition_size
        );
    ...
    async fn test_node_recovery(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if self.nodes.len() > 1 {
                ...
                .is_ok()
            {
                ...
                self.record_test_failure(
                    "node_recovery",
                    "Failed to perform operations after recovery",
                )
                .await;
            }
        ...
        if successful_operations == self.config.concurrent_operations {
            ...
            self.record_test_failure(
                ...
                &format!(
                    ...
                    successful_operations, self.config.concurrent_operations
                ),
            )
            .await;
        }
    ...
    async fn test_data_integrity(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for (filename, content) in test_files {
            ...
            if let Some(node) = self.nodes.first() {
                if self
                    .simulate_file_store(node, filename, &content)
                    .await
                    .is_ok()
                {
                    // Verify file integrity
                    if self
                        .simulate_file_integrity_check(node, filename)
                        .await
                        .unwrap_or(false)
                    {
                        self.record_test_success("data_integrity").await;
                    } else {
                        self.record_test_failure("data_integrity", "Integrity check failed")
                            .await;
                    }
                }
            }
    ...
    async fn simulate_file_retrieve(
        &self,
        node: &TestNode,
        filename: &str,
    ...
    async fn simulate_file_integrity_check(
        &self,
        _node: &TestNode,
        _filename: &str,
    ) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    async fn record_test_failure(&self, test_name: &str, error_msg: &str) {
        ...
        results
            .error_log
            .push(format!("{}: {}", test_name, error_msg));
    }
}
