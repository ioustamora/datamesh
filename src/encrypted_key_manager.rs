...
impl EncryptedKeyManager {
    ...
    pub fn new(key: &SecretKey, name: String, password: &str) -> Result<Self> {
        ...
        let ciphertext = cipher.encrypt(nonce_array, key_data.as_ref())
        ...
        let encrypted_integrity = cipher.encrypt(integrity_nonce_array, &integrity_hash.as_bytes()[..])
    ...
    pub fn load_from_file(keys_dir: &Path, name: &str, password: &str) -> Result<KeyManager> {
        ...
        if !key_file.exists() {
            return Err(anyhow::anyhow!("Key file not found: {}", key_file.display()));
        }
        ...
        let decrypted_integrity = cipher.decrypt(integrity_nonce, encrypted_key.integrity_hash.as_ref())
    ...
    pub fn change_password(keys_dir: &Path, name: &str, old_password: &str, new_password: &str) -> Result<()> {
...
mod tests {
    ...
    fn test_encrypted_key_creation_and_loading() {
        ...
        // Load from file
        let loaded_manager = EncryptedKeyManager::load_from_file(keys_dir, "test_key", password).unwrap();
    ...
    fn test_password_change() {
        ...
        let encrypted_manager = EncryptedKeyManager::new(&secret_key, "test_key".to_string(), old_password).unwrap();
        encrypted_manager.save_to_file(keys_dir).unwrap();
        
        // Change password
        EncryptedKeyManager::change_password(keys_dir, "test_key", old_password, new_password).unwrap();
        
        // Verify old password no longer works
        assert!(EncryptedKeyManager::load_from_file(keys_dir, "test_key", old_password).is_err());
        
        // Verify new password works
        let loaded_manager = EncryptedKeyManager::load_from_file(keys_dir, "test_key", new_password).unwrap();
        assert_eq!(secret_key.serialize(), loaded_manager.key.serialize());
    }
    ...
    fn test_legacy_key_migration() {
        ...
        let loaded_manager = EncryptedKeyManager::load_from_file(keys_dir, "legacy_key", password).unwrap();
        assert_eq!(secret_key.serialize(), loaded_manager.key.serialize());
