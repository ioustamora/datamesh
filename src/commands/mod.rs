...
pub trait CommandHandler: Send + Sync {
    ...
    async fn execute_with_monitoring(&self, context: &CommandContext) -> Result<(), Box<dyn Error>> {
...
pub fn create_command_handler(command: &Commands) -> Box<dyn CommandHandler> {
    match command {
        // File operations
        Commands::Put { path, public_key, name, tags } => {
        ...
        Commands::Info { identifier } => {
            Box::new(file_commands::InfoCommand {
                identifier: identifier.clone(),
            })
        }
        Commands::Stats => {
            Box::new(file_commands::StatsCommand)
        }
        ...
        Commands::Bootstrap { port } => {
            Box::new(service_commands::BootstrapCommand {
                port: *port,
            })
        }
        ...
        Commands::Metrics { summary, export } => {
            Box::new(admin_commands::MetricsCommand {
                summary: *summary,
                export: *export,
            })
        }
        Commands::Networks => {
            Box::new(admin_commands::NetworksCommand)
        }
        
        // Advanced operations - delegate to existing advanced_commands module
        _ => {
            Box::new(advanced_commands::AdvancedCommandHandler {
