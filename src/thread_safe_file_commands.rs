...
impl ThreadSafeFileCommands {
    ...
    pub async fn new(cli: Cli, key_manager: Arc<KeyManager>, config: Arc<Config>) -> Result<Self, Box<dyn Error>> {
    ...
    pub async fn put_file(
        ...
        self.context.store_file(path, public_key, name, tags).await
    ...
    pub async fn get_file(
        ...
        self.context.retrieve_file(identifier, output_path, private_key).await
            .map_err(|e| Box::new(e) as Box<dyn Error>)
    }
}
...
impl CommandHandler for ThreadSafePutCommand {
    async fn execute(&self, _context: &CommandContext) -> Result<(), Box<dyn Error>> {
        let result = self.commands.put_file(&self.path, &self.public_key, &self.name, &self.tags).await?;
...
impl CommandHandler for ThreadSafeGetCommand {
    async fn execute(&self, _context: &CommandContext) -> Result<(), Box<dyn Error>> {
        self.commands.get_file(&self.identifier, &self.output_path, &self.private_key).await?;
