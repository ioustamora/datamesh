        ...
        let (
            ...
            if let Some(peer_stat) = stats.get(peer_id) {
                let avg_time = if peer_stat.response_times.is_empty() {
                    ...
                    peer_stat.response_times.iter().sum::<u64>()
                        / peer_stat.response_times.len() as u64
                };

                let duration = peer_stat.connected_at.elapsed();
                let connected_time =
                    Local::now() - chrono::Duration::milliseconds(duration.as_millis() as i64);
...
async fn collect_network_health_with_diagnostics(
    ...
    debug!(
        "Network health: {} peers connected, {}% uptime, {} stability score",
...
async fn analyze_network_topology(
    ...
    info!(
        "Network topology analysis: {} neighbors, {} routing buckets, estimated diameter: {}",
        neighbors.len(),
        routing_buckets.len(),
        estimated_diameter
    );
...
async fn test_peer_bandwidth(
    ...
    let (download_speed, upload_speed, packet_loss) =
        perform_bandwidth_test(swarm, peer_id, test_duration).await?;
    ...
    info!(
        "Bandwidth test completed: download={} bytes/s, upload={} bytes/s, rtt={}ms",
...
async fn measure_peer_rtt(swarm: &mut Swarm<MyBehaviour>, peer_id: PeerId) -> Result<u64> {
    ...
    let rtt = timeout(timeout_duration, async {
        ...
        tokio::time::sleep(Duration::from_millis(
            10 + (peer_id.to_bytes()[0] as u64 % 100),
        ))
        .await;
        start.elapsed().as_millis() as u64
    })
    .await
    .unwrap_or(5000); // Default to 5 second timeout
...
async fn measure_peer_latency(
    ...
    info!(
        "Measuring latency to peer {} with {} samples",
        peer_id, samples
    );
    ...
    info!(
        "Latency measurement complete: avg={}ms, p95={}ms, loss={}%",
...
async fn measure_all_peer_latency(
    ...
    info!(
        "Measuring latency to {} peers with {} samples each",
        peer_ids.len(),
        samples
    );
    ...
    info!(
        "Network latency measurement complete: avg={}ms, p95={}ms, loss={}%",
        stats.avg_latency, stats.p95_latency, stats.packet_loss_rate
    );
...
impl NetworkDiagnostics {
    ...
    pub fn get_network_latency_stats(&self) -> LatencyStats {
        ...
        calculate_latency_statistics(all_latencies, 0, len).unwrap_or_else(|_| LatencyStats {
            peer_id: None,
            samples: 0,
            min_latency: 0,
            max_latency: 0,
