...
pub async fn repair_files(
    ...
    let files_to_repair = if let Some(target) = &config.target {
        ...
        return Err(anyhow::anyhow!("Either specify a target file or use --auto"));
    };
...
pub async fn cleanup_storage(config: CleanupConfig) -> Result<()> {
    ...
    if config.orphaned {
        ...
        cleanup_actions.push(format!("Removed {} of orphaned data", ui::format_file_size(orphaned_size)));
    }
    ...
    if config.duplicates {
        ...
        if !duplicates.is_empty() {
            ...
            } else if config.force || ui::confirm_action("Remove duplicate files (keeping newest)?", false) {
    ...
    if config.low_health {
        ...
        if !low_health_files.is_empty() {
            ui::print_warning(&format!("Found {} files with irreparable low health", low_health_files.len()));
            
            if config.dry_run {
                for file in &low_health_files {
                    println!("  Would remove: {} ({}% health)", 
                            file.name,
                ...
                total_space_freed += removed_size;
                cleanup_actions.push(format!("Removed {} low-health files", low_health_files.len()));
            }
        } else {
    ...
    ui::print_key_value("Total space freed", &ui::format_file_size(total_space_freed));
...
pub async fn manage_quota(
    ...
    if let Some(limit_str) = limit {
        ...
        ui::print_success(&format!("Storage limit set to {}", ui::format_file_size(new_limit)));
    }
...
pub async fn run_benchmarks(
    ...
    if full || storage {
        ...
        ui::print_key_value("Write Speed", &format!("{:.1} MB/s", results.storage_write_speed));
        ...
        ui::print_key_value("Read Speed", &format!("{:.1} MB/s", results.storage_read_speed));
        
        // Test database performance
        ui::print_operation_status("Database", "Testing", None);
        results.database_query_time = benchmark_database_performance().await?;
        ui::print_key_value("Query Time", &format!("{:.2}ms", results.database_query_time.as_millis()));
    }
    ...
    if full {
        ...
        ui::print_key_value("Encryption Speed", &format!("{:.1} MB/s", results.encryption_speed));
        ...
        ui::print_key_value("Chunk Processing", &format!("{:.1} MB/s", results.chunk_processing_speed));
    }
    
    ui::print_section("Benchmark Summary");
    if results.storage_write_speed > 0.0 {
        ui::print_key_value("Storage Write", &format!("{:.1} MB/s", results.storage_write_speed));
    }
    if results.storage_read_speed > 0.0 {
        ui::print_key_value("Storage Read", &format!("{:.1} MB/s", results.storage_read_speed));
    }
    if results.encryption_speed > 0.0 {
        ui::print_key_value("Encryption", &format!("{:.1} MB/s", results.encryption_speed));
    }
...
async fn repair_single_file(
    ...
    if file.chunks_healthy < 4 {
        return Err(anyhow::anyhow!("Cannot repair file with less than 4 healthy chunks"));
    }
...
async fn remove_duplicate_files(_db: &DatabaseManager, _duplicates: &[(String, Vec<String>)]) -> Result<u64> {
...
fn display_quota_usage(quota_info: &QuotaInfo) {
    ...
    ui::print_key_value("Current usage", &ui::format_file_size(quota_info.current_usage));
    ui::print_key_value("File count", &quota_info.file_count.to_string());
    
    if let Some(limit) = quota_info.limit {
        ui::print_key_value("Storage limit", &ui::format_file_size(limit));
        ui::print_key_value("Usage percentage", &format!("{:.1}%", quota_info.usage_percentage));
    }
...
fn display_health_report(report: &HealthReport) {
    ...
    if !report.duplicate_files.is_empty() {
        ui::print_warning(&format!("Found {} duplicate file groups", report.duplicate_files.len()));
    }
}

// Benchmark helper functions

async fn benchmark_storage_write(_cli: &Cli, _key_manager: &KeyManager, duration: u64) -> Result<f64> {
...
async fn benchmark_storage_read(_cli: &Cli, _key_manager: &KeyManager, duration: u64) -> Result<f64> {
    use std::time::Instant;
    use std::io::Read;
...
async fn benchmark_chunk_processing(duration: u64) -> Result<f64> {
    ...
    while start_time.elapsed().as_secs() < duration {
        ...
        for i in 0..6 {
            ...
            let end_idx = if i == 5 { test_data.len() } else { (i + 1) * shard_size };
