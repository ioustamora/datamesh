...
impl ErrorBatch {
    ...
    pub fn has_critical_errors(&self) -> bool {
        self.errors
            .iter()
            .any(|e| get_error_severity(&e.error) == ErrorSeverity::Critical)
    }
...
pub fn get_error_severity(error: &DfsError) -> ErrorSeverity {
    match error {
        ...
        DfsError::Internal(_) => ErrorSeverity::Warning,
    }
}
...
pub fn handle_error(error: &(dyn std::error::Error + 'static)) -> EnhancedError {
    ...
    let dfs_error = if let Some(io_err) = error.downcast_ref::<std::io::Error>() {
        ...
        match dfs_err {
            ...
            DfsError::Internal(msg) => DfsError::Internal(msg.clone()),
        }
    } else {
...
pub fn add_context_and_suggestions(mut error: EnhancedError) -> EnhancedError {
    match &error.error {
        ...
        _ => {
            error = error.with_suggestions(vec![
                "Try the operation again".to_string(),
...
macro_rules! enhanced_error {
    ($error_type:expr, $msg:expr) => {
        crate::error_handling::add_context_and_suggestions(crate::error::EnhancedError::new(
            $error_type,
        ))
    };
    ($error_type:expr, $msg:expr, $context:expr) => {
        crate::error_handling::add_context_and_suggestions(
            crate::error::EnhancedError::new($error_type).with_context($context.to_string()),
        )
    };
}
...
pub fn network_error_with_suggestions(message: &str) -> EnhancedError {
    add_context_and_suggestions(EnhancedError::new(DfsError::Network(message.to_string())))
}
...
pub fn file_not_found_error_with_suggestions(message: &str) -> EnhancedError {
    add_context_and_suggestions(EnhancedError::new(DfsError::FileNotFound(
        message.to_string(),
    )))
}
...
pub fn database_error_with_suggestions(message: &str) -> EnhancedError {
    add_context_and_suggestions(EnhancedError::new(DfsError::Database(message.to_string())))
}

/// Helper function to create contextual error for specific operations
pub fn operation_error_with_context(
    operation: &str,
    ...
    match operation {
        "put" => {
            ...
            if !enhanced
                .suggestions
                .iter()
                .any(|s| s.contains("file size") || s.contains("permissions"))
            {
